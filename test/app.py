...
@app.route('/get-legend-count', methods=['POST'])
def get_legend_count():
    ...
    try:
        ...
        # ✅ COMMON FILTER
        # =========================
        where_clause = "a.`Area ID` = %s"
        params = [selected_area]

        if user_id:
            where_clause += " AND a.`User ID` = %s"
            params.append(user_id)

        if start_date and end_date:
            where_clause += " AND a.`Task Date` BETWEEN %s AND %s"
        ...
        query = f"""
            SELECT
                ...
                -- 🟢 Remedy counts (only valid text)
                SUM(CASE 
                    WHEN a.`Assessment Case` = 'Case1'
                     AND r.`Remedy Text` IS NOT NULL 
                     AND TRIM(r.`Remedy Text`) <> ''
                    THEN 1 ELSE 0 
                END) AS remedy_case1,

                SUM(CASE 
                    WHEN a.`Assessment Case` = 'Case2'
                     AND r.`Remedy Text` IS NOT NULL 
                     AND TRIM(r.`Remedy Text`) <> ''
                    THEN 1 ELSE 0 
                END) AS remedy_case2,
        ...
        response = {
            ...
            # 🟢 Remedy
            "remedy_case1": result[4] or 0,
            "remedy_case2": result[5] or 0,
            "remedy_case3": result[6] or 0,
            "remedy_case4": result[7] or 0
        }
...
def get_chart_data():
    ...
    try:
        ...
        cursor.execute("""
     SELECT 
    YEAR(`Task Date`) AS year,
    MONTHNAME(`Task Date`) AS month,
    COUNT(*) AS count
FROM assessment
WHERE YEAR(`Task Date`) = 2026
GROUP BY YEAR(`Task Date`), MONTH(`Task Date`)
...
def upload_images():
    try:
        ...
        pile_no = request.form.get('pile_no')

        if not assessment_id:
            return jsonify({"success": False, "error": "Missing assessment_id"}), 400

        # 📁 Folder path
        base_path = os.path.join("C:/Users/LENOVO/Desktop/AssessmentPictures", assessment_id)
        ...
        cursor.execute("""
            SELECT `Picture1 Name`, `Picture2 Name`,
                   `Picture3 Name`, `Picture4 Name`
            FROM assessment
            WHERE `Assessment ID`=%s
              AND `Table ID`=%s
              AND `Pile No`=%s
        """, (assessment_id, table_id, pile_no))

        row = cursor.fetchone()

        if row:
            image_names = list(row)
        else:
            image_names = [None, None, None, None]

        # ✅ STEP 2: Replace only uploaded images
        for i in range(1, 5):
            file = request.files.get(f"side{i}")

            if file and file.filename:
                filename = f"{assessment_id}_{table_id}_Pile{pile_no}_side{i}.jpg"
                file_path = os.path.join(base_path, filename)

                file.save(file_path)   # 🔥 THIS AUTOMATICALLY REPLACES OLD FILE

                image_names[i-1] = filename
...
def get_final_submitted_hotspots():
    ...
    try:
        cursor.execute("SELECT DISTINCT `Table ID` FROM `assessment` WHERE `Assessment Status` = 'OE Approved'")  
        rows = cursor.fetchall()
...
def get_submitted_hotspots_remedy():
    ...
    try:
        # 👇 Get distinct values
        cursor.execute("SELECT DISTINCT `Table ID` FROM `remedy`")  
        rows = cursor.fetchall()

        submitted_hotspots = []
        for row in rows:
            if row['Table ID']:
                submitted_hotspots.extend(row['Table ID'].split(","))

        # 👇 Optional: remove duplicates again after split
        submitted_hotspots = list(set(submitted_hotspots))
...
@app.route('/get_all_piles_concreted', methods=['GET'])
def get_all_piles_concreted():
    connection = get_db_connection()
    cursor = connection.cursor(dictionary=True)
    ...
    try:
        query = """
            ...
            HAVING 
                COUNT(CASE 
                    WHEN a.`Assessment case` IN ('Case1','Case2','Case3') 
                    THEN 1 
                END)
                =
                COUNT(CASE 
        ...
        tables = [row['Table ID'] for row in rows]
...
@app.route('/get_remedy_text_count', methods=['GET'])
def get_remedy_text_count():
    connection = get_db_connection()
    cursor = connection.cursor(dictionary=True)
